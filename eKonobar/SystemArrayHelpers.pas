...
type
  ...
  public
    ...
    class function AddValueUnique(const x: T; var Values: TArray<T>) : integer; overload;
    ...
    class procedure DeleteValue(const Element: T; var Values: TArray<T>);
...
class function TArrayUtils<T>.AddValueUnique(const x: T;
  var Values: TArray<T>): integer;
begin
  Result := -1;
  if not Contains(x, Values) then
    Result := AddValue(x, Values);
end;
...
class procedure TArrayUtils<T>.DeleteValue(const Element: T;
  var Values: TArray<T>);
begin
...
class function TArrayUtils<T>.GetIndex(const Values: TArray<T>; Callback: TArrayFindItemCallback): integer;
begin
  Result := -1;
  for var I := 0 to High(Values) do
    if Callback( Values[I] ) then
...
begin
  ...
  begin
    ...
    while i <= j do
    begin
      while (i <= AHigh) and Callback(Pivot, Values[i]) do
        Inc(i);
      while (j >= 0) and Callback(Values[j], Pivot) do
        Dec(j);
      if i <= j then
      begin
        Temp := Values[i];
